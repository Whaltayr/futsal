...
// Caminho estável no filesystem
$projectRoot = dirname(__DIR__);
$uploadBase = $projectRoot . '/uploads/teams';

// Debug temporário (remova em produção)
error_log('DEBUG teams_actions: uploadBase='.$uploadBase.' docroot='.( $_SERVER['DOCUMENT_ROOT'] ?? '' ));
...
try {
  switch ($action) {
    case 'create':
      ...
      if ($tournament_id <= 0 || $name === '') { http_response_code(400); echo json_encode(['erro'=>'Dados inválidos']); exit; }
    ...
    case 'delete':
      ...
      if (!empty($r['logo_url']) && file_exists(rtrim($_SERVER['DOCUMENT_ROOT'],'/') . $r['logo_url'])) {
        @unlink(rtrim($_SERVER['DOCUMENT_ROOT'],'/') . $r['logo_url']);
      }
  ...
  error_log('teams_actions exception: '.$e->getMessage().' @ '.$e->getFile().':'.$e->getLine());
